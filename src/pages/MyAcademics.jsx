...
export default function MyAcademics({ user }) {
      ...
      try {
        ...
        if (isMounted && data) {
          ...
          if (data.semesters && data.semesters.length > 0) {
            const selected = data.currentSemesterId && data.semesters.some(s => s.id === data.currentSemesterId)
              ? data.currentSemesterId
              : data.semesters[0].id;
            setActiveSemesterId(selected);
          }
        }
      } catch (err) {
  ...
  const activeSemester = useMemo(() => {
    return academicData.semesters.find(s => s.id === activeSemesterId) || null;
  ...
  const persistChanges = async (newData) => {
    ...
    setIsSaving(true);
    const success = await saveStudentAcademics(userId, newData, studentName);
    setIsSaving(false);
    return success;
  };

  // ─── SEMESTER ACTIONS ────────────────────────────────────────────────────────
  const handleAddSemester = async (presetName = null) => {
    const name = (presetName || newSemesterName).trim();
    if (!name) {
  ...
  const handleUpdateSemesterSgpa = async () => {
    ...
    addToast({ message: 'SGPA updated successfully!', type: 'success' });
  };

  const handleUpdateOverallCgpa = async () => {
    ...
    addToast({ message: 'Overall CGPA updated successfully!', type: 'success' });
  };
  ...
  const openEditSubjectModal = (sub) => {
    ...
    setSubjectForm({
      ...
      mid1Max: sub.mid1?.max !== undefined && sub.mid1?.max !== null ? String(sub.mid1.max) : '30',
      ...
      attendancePercentage: sub.attendance?.value !== undefined && sub.attendance?.value !== null ? String(sub.attendance.value) : '',
  ...
  const validateSubjectForm = () => {
    ...
    if (subjectForm.attendanceMode === 'percentage') {
      if (subjectForm.attendancePercentage.trim() !== '') {
      ...
      if (subjectForm.classesTotal.trim() !== '' || subjectForm.classesAttended.trim() !== '') {
  ...
  const handleSaveSubject = async (e) => {
    ...
    const formatMarkObj = (obtainedStr, maxStr) => {
      if (obtainedStr.trim() === '') return null;
      return {
    ...
    const subjectPayload = {
      ...
      mid1: formatMarkObj(subjectForm.mid1Obtained, subjectForm.mid1Max),
      mid2: formatMarkObj(subjectForm.mid2Obtained, subjectForm.mid2Max),
      semMarks: formatMarkObj(subjectForm.semObtained, subjectForm.semMax),
      attendance: attendanceObj
    };

    let updatedSubjects = [];
    if (editingSubject) {
      updatedSubjects = activeSemester.subjects.map(s => s.id === editingSubject.id ? subjectPayload : s);
    } else {
      updatedSubjects = [...(activeSemester.subjects || []), subjectPayload];
    }
            ...
            <div className="glass-panel border border-white/10 rounded-3xl p-5 relative overflow-hidden group hover:border-emerald-500/30 transition-all">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className={'px-2 py-0.5 rounded-md text-[10px] font-black uppercase border ' + getAttendanceBadge(overallAttendance).bg}>
                  {overallAttendance >= 75 ? 'Safe' : overallAttendance >= 65 ? 'Warning' : 'Low'}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/5 pb-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-gray-400">
                    Semester-wise SGPA trajectory and performance trends.
                  </p>
                </div>
            ...
            {activeSemester && (
              <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="px-4 py-2 rounded-2xl bg-white/5 border border-white/10 text-center">
                      ...
                      <span className={'text-base font-black ' + (
                        currentSemesterAttendance >= 75 ? 'text-emerald-400' : currentSemesterAttendance >= 65 ? 'text-amber-400' : 'text-rose-400'
                      )}>
                ...
                {(!activeSemester.subjects || activeSemester.subjects.length === 0) ? (
                  <div className="glass-panel border border-dashed border-white/15 rounded-3xl p-8 sm:p-12 text-center space-y-4">
                    ...
                    <button
                      onClick={openAddSubjectModal}
                      className="px-6 py-2.5 rounded-2xl bg-brand-teal text-[#030712] font-black text-xs hover:bg-brand-teal/90 transition-all inline-flex items-center gap-1.5 cursor-pointer"
                    >
                      <Plus className="w-4 h-4 stroke-[3]" />
                      <span>+ Add Subject</span>
                    </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {activeSemester.subjects.map(sub => {
                      ...
                      return (
                        <div
                          key={sub.id}
                          className="glass-panel border border-white/10 rounded-3xl p-5 space-y-4 hover:border-brand-teal/30 transition-all flex flex-col justify-between relative group"
                        >
                          {/* Subject Card Header */}
                            ...
                            <div className="space-y-0.5">
                              ...
                              <div className="flex items-center gap-1.5">
                                <span className="text-xs font-black text-white">
                                  {attPct > 0 ? attPct + '%' : 'Not recorded'}
                                </span>
                                {sub.attendance?.type === 'classes' && sub.attendance?.total > 0 && (
          ...
          >
            ...
            <div className="space-y-3 pt-3 border-t border-white/5">
              ...
              <button
                ...
                disabled={!newSemesterName.trim() || isSaving}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-white/10">
              ...
              <button
                onClick={handleUpdateOverallCgpa}
                disabled={isSaving}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-white/10">
              ...
              <button
                onClick={handleUpdateSemesterSgpa}
                disabled={isSaving}
