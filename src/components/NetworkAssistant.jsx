        ...
        <div className="flex gap-1 bg-black/35 p-1 rounded-2xl border border-white/5 self-start sm:self-auto">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-semibold tracking-wide transition-all cursor-pointer ${
            ...
            {activeAnalysisType === 'study' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {matches.study.map((m, idx) => (
                  <div key={idx} className="p-4 bg-white/5 border border-white/5 hover:border-white/10 rounded-2xl space-y-3 transition-all">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <span className="text-[10px] text-green-400 font-semibold uppercase bg-green-500/10 border border-green-500/20 px-2 py-0.5 rounded leading-none shrink-0">
                        {m.rating}
                      </span>
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full bg-[#f72585]/10 hover:bg-[#f72585]/20 text-brand-pink font-extrabold py-2 rounded-xl text-[10px] tracking-wide transition-colors cursor-pointer border border-[#f72585]/20"
                    >
                      Connect & Study
                    </button>
                  </div>
                ...
                {matches.hackathon.map((m, idx) => (
                  <div key={idx} className="p-4 bg-white/5 border border-white/5 hover:border-white/10 rounded-2xl space-y-3 transition-all">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <span className="text-[9px] text-brand-pink font-semibold uppercase bg-brand-pink/10 border border-white/10 px-2.5 py-1 rounded-xl shrink-0">
                        {m.role}
                      </span>
                    </div>
                    <p className="text-[11px] text-gray-400 leading-relaxed font-normal">{m.reason}</p>
                    <button 
                      ...
                      className="w-full bg-white/5 hover:bg-white/10 text-gray-200 font-extrabold py-2 rounded-xl text-[10px] tracking-wide transition-colors cursor-pointer border border-white/10"
                    >
                ...
                {matches.mentor.map((m, idx) => (
                  <div key={idx} className="p-4 bg-white/5 border border-white/5 hover:border-white/10 rounded-2xl space-y-3 transition-all">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <span className="text-[9px] text-brand-blue font-semibold uppercase bg-brand-blue/10 border border-white/10 px-2.5 py-1 rounded-xl shrink-0">
                        {m.area}
                      </span>
                    </div>
                    <p className="text-[11px] text-gray-400 leading-relaxed font-normal">{m.reason}</p>
                    <button 
                      ...
                      className="w-full bg-brand-teal text-black font-extrabold py-2 rounded-xl text-[10px] tracking-wide transition-colors cursor-pointer"
                    >
